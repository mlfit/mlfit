...
Since the fitted weights contain decimals, we need to integerise them in order to generate a synthetic population because it wouldn't make sense to create half a person.
The package provides `ml_integerise()` to help you with this task.
`ml_integerise()` returns a `ml_fit` object with a field that contains integerised weights.
...
You can use `ml_evaluate()` to access the fit and the integerised fitted weights across different matrices such as the G statistic, sum of squared errors, standardised root mean squared errors, and R2.

```{r}
dplyr::bind_rows(ml_evaluate(fit), ml_evaluate(int_fit))
```

Lastly, `ml_replicate()`, as its name suggests, helps to replicate the reference sample based on the integerised weights.
